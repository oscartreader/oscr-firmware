...
namespace OSCR::Cores::PokemonMini
{
  ...
  void menu()
  {
    ...
    do
    {
      switch (OSCR::UI::menu(FS(OSCR::Strings::Cores::PokemonMini), menuOptions, sizeofarray(menuOptions)))
      {
      case 0: // Read ROM
        readROM();
        break;
  ...
  void printHeader()
  {
    OSCR::UI::printHeader(FS(OSCR::Strings::Cores::PokemonMini));
  }
  ...
  void openCRDB()
  {
    OSCR::Databases::Basic::setup(FS(OSCR::Strings::FileType::PokemonMini));
  }
  ...
  void readROM()
  {
    ...
    OSCR::Storage::Shared::createFile(FS(OSCR::Strings::FileType::PokemonMini), FS(OSCR::Strings::Directory::ROM), fileName);

    cartOn();

    OSCR::UI::ProgressBar::init(0x80000);

    for (uint32_t addr = 0; addr < 0x80000; addr += 512)
    {
      for (int w = 0; w < 512; w++)
      {
