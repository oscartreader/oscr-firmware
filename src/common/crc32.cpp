...
namespace OSCR
{
  namespace CRC32
  {
...
#if OPTION_CRC32_LUT
    constexpr uint32_t const crc_32_tab[] = {
#else
    constexpr uint32_t const PROGMEM crc_32_tab[] = {
#endif
    ...
    uint32_t calculateCRC(uint8_t const * const buffer, size_t length)
    {
      current.reset();
    ...
    uint32_t calculateCRC(OSCR::Storage::File & infile)
    {
      uint32_t length;

      infile.assignBuffer(BUFFN(OSCR::Storage::Shared::buffer));

      current.reset();
      while ((length = infile.fill()) != 0)
      {
        for (size_t i = 0; i < length; ++i)
        {
          current += OSCR::Storage::Shared::buffer[i];
        }
      }
      current.done();
      return current;
    ...
    uint32_t calculateCRC(char const * const fileName, char const * const folder, uint32_t offset)
    {
      OSCR::Storage::File infile;
      uint32_t result;
    ...
    crc32_t::crc32_t(): value(0xFFFFFFFF)
    {
      // ...
    }

    crc32_t::crc32_t(uint32_t v): value{v}
    {
      // ...
    }

    crc32_t::operator uint32_t() const
    {
      return value;
    }
    ...
    __optimize_crc32
    crc32_t & crc32_t::operator+=(uint16_t data)
    {
    ...
    __optimize_crc32
    crc32_t & crc32_t::operator+=(uint32_t data)
    {
    ...
    __optimize_crc32
    void crc32_t::next(uint8_t const * data)
    {
      update(value, *data);
    }

    __optimize_crc32
    void crc32_t::next(uint8_t data)
    {
      update(value, data);
    }
    ...
    void crc32_t::human(char * buffer, uint_fast8_t sizeOf) const
    {
      snprintf_P(buffer, sizeOf, PSTR("%02X%02X%02X%02X"), b[3], b[2], b[1], b[0]);
    }
